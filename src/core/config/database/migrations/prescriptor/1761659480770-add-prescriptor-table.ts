...
export class AddPrescriptorTable1761659480770 implements MigrationInterface {
    name = 'AddPrescriptorTable1761659480770'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "pharmasys"."prescriptor" ("id" SERIAL NOT NULL, "uuid" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), "status" "pharmasys"."prescriptor_status_enum" NOT NULL DEFAULT 'A', "name" character varying(150) NOT NULL, "registration_number" character varying(30) NOT NULL, "specialty" character varying(150), "state" character(2) NOT NULL, "user_created_id" integer, "user_updated_id" integer, "advice_id" integer, CONSTRAINT "PK_410772504ec7a1ea828e3f09e21" PRIMARY KEY ("id")); COMMENT ON COLUMN "pharmasys"."prescriptor"."uuid" IS 'Identificador único universal'; COMMENT ON COLUMN "pharmasys"."prescriptor"."status" IS 'Status da entidade (A-Ativo, I-Inativo)'; COMMENT ON COLUMN "pharmasys"."prescriptor"."name" IS 'Nome do prescritor'; COMMENT ON COLUMN "pharmasys"."prescriptor"."registration_number" IS 'Número de registro do prescritor no conselho profissional'; COMMENT ON COLUMN "pharmasys"."prescriptor"."specialty" IS 'Especialidade do prescritor'; COMMENT ON COLUMN "pharmasys"."prescriptor"."state" IS 'UF do conselho profissional'`);
