...
export class AddStockTransferModule1761681675119 implements MigrationInterface {
    name = 'AddStockTransferModule1761681675119'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "pharmasys"."stock_transfer_status_enum" AS ENUM('A', 'I')`);
        await queryRunner.query(`CREATE TABLE "pharmasys"."stock_transfer" ("id" SERIAL NOT NULL, "uuid" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), "status" "pharmasys"."stock_transfer_status_enum" NOT NULL DEFAULT 'A', "transfer_date" TIMESTAMP WITH TIME ZONE NOT NULL, "user_created_id" integer, "user_updated_id" integer, "origin_id" integer, "destination_id" integer, CONSTRAINT "UQ_fc554b7666c0b2b924ecff905f1" UNIQUE ("uuid"), CONSTRAINT "PK_b6165ea3cc5b8062e7eaa1bd44d" PRIMARY KEY ("id")); COMMENT ON COLUMN "pharmasys"."stock_transfer"."uuid" IS 'Identificador único universal'; COMMENT ON COLUMN "pharmasys"."stock_transfer"."status" IS 'Status da entidade (A-Ativo, I-Inativo)'; COMMENT ON COLUMN "pharmasys"."stock_transfer"."transfer_date" IS 'Data de transferência selecionada pelo usuário'`);
