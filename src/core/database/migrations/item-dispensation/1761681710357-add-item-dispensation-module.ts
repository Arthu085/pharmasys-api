...
export class AddItemDispensationModule1761681710357 implements MigrationInterface {
    name = 'AddItemDispensationModule1761681710357'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "pharmasys"."item_dispensation_status_enum" AS ENUM('A', 'I')`);
        await queryRunner.query(`CREATE TABLE "pharmasys"."item_dispensation" ("id" SERIAL NOT NULL, "uuid" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE DEFAULT now(), "status" "pharmasys"."item_dispensation_status_enum" NOT NULL DEFAULT 'A', "dispensation_date" TIMESTAMP WITH TIME ZONE NOT NULL, "user_created_id" integer, "user_updated_id" integer, "patient_id" integer, "prescriptor_id" integer, CONSTRAINT "UQ_9f07d6c0e429ce4bdb2584ccba5" UNIQUE ("uuid"), CONSTRAINT "PK_4cfe6ce4f87f57b675bf0a1a881" PRIMARY KEY ("id")); COMMENT ON COLUMN "pharmasys"."item_dispensation"."uuid" IS 'Identificador único universal'; COMMENT ON COLUMN "pharmasys"."item_dispensation"."status" IS 'Status da entidade (A-Ativo, I-Inativo)'; COMMENT ON COLUMN "pharmasys"."item_dispensation"."dispensation_date" IS 'Data da dispensação selecionada pelo usuário'`);
